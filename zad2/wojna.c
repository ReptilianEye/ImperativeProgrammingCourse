...
{ // object used to represent Player; their hand and table (as objected as it possible)
...
int simul_game(rbuffer *p1, rbuffer *p2, int easy, int max_conflicts)
{
    ...
    { // if limit of conflicts is reached cards on table returns to hands of the players
        assign_cards(p1, p1);
        assign_cards(p2, p2);
        return 0;
    }
    if (hand_state(p2) == 0)
    {              // handling winning of 1st player
