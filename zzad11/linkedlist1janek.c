...
void pushFront(List *list, void *data) {
    //ListElement previously_first = list->head;
    if (list->head == NULL) {
...
ListElement* findInsertionPoint(const List *list, ListElement *element) {
    ...
    if (current == NULL) {
        ...
        if (current->data == element-> data) {return NULL;};
    }
    if (current == NULL) {
        return list->tail;
    }
...
void insertInOrder(List *list, void *a) {
    ListElement *new_n = malloc (sizeof(ListElement));
    new_n->data = a;
    ListElement* x = findInsertionPoint (list, new_n);
